...
const BlackVictimDataComponent = () => {
  ...
  const fetchData = async (cityName) => {
    const query = `
      PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
      PREFIX project-2: <http://www.semanticweb.org/vchavhan/ontologies/2023/10/project-2#>
      SELECT (ROUND((COUNT(DISTINCT ?blackVictim) / COUNT(DISTINCT ?victim)) * 100, 2) AS ?percentageBlack)
      WHERE {
        ?victim rdf:type project-2:victim .
        ?victim project-2:hasCityName "${cityName}" .
        BIND(IF(EXISTS { ?victim project-2:hasBlackRacePercentageForCity ?blackValue }, true, false) AS ?blackVictim)
      }
    `;
    ...
    try {
      ...
      const cityData = response.data.results.bindings.map(binding => ({
        city: cityName,
        percentageBlack: binding.percentageBlack.value,
      }));
  ...
  return (
    <div className="flex flex-col items-center min-h-screen my-4 p-4">
      <div className="text-center">
        <h2 className="text-xl font-semibold text-center">Discover Black Victim Percentage in U.S. Cities</h2>
        <p className="mb-4">
        Use this tool to search for any U.S. city and discover the percentage of Black victims. Start typing the city name in the input field below, select a city from the suggestions, and view the results.
</p>
      </div>
      ...
      {selectedCityData && (
        <div className="mt-4 text-center">
          ...
          <p className="text-lg">{`Black people are ${selectedCityData.percentageBlack}% of the victims in ${selectedCityData.city}.`}</p>
        </div>
      )}
    </div>
  );
...
export default BlackVictimDataComponent;
